...
public class SimpleTestFragment extends AbstractTestFragment implements ServiceConnection
{
    ...
    private View createView(final View view, final LayoutInflater inflater, final Bundle savedInstanceState) {
        ...
        if (testView != null) {
            ...
            final View topStatusBarView = view.findViewById(R.id.simple_text_status_top_bar);
            testView.setTopStatusBar(topStatusBarView);
        }
        ...
        public void run()
        {
            ...
            if (rmbtService != null) {
                if (rmbtService.isCompleted() && rmbtService.getTestUuid(false) != null && !rmbtService.isConnectionError()) {
                    if (SimpleTestFragment.this.intermediateResult == null) {
                        SimpleTestFragment.this.intermediateResult = rmbtService.getLastIntermediateResult();
                    }
                    SimpleTestFragment.this.lastIntermediateResult = rmbtService.getLastIntermediateResult();
    ...
    private void switchToResult()
    {
        ...
        if (tempResult.status != TestStatus.ERROR) {
            ((RMBTMainActivity) getActivity()).showStartScreenAfterTestWithoutSwitchingToResults(testUuid, tempResult/*rmbtService.getIntermediateResult(intermediateResult)*/);
        }
    }
