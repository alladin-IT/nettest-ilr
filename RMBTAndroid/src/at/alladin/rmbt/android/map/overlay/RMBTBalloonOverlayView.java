            ...
            for (int i = 0; i < 1; i++)
                ...
                try
                {
                    ...
                    if (result.has("timestamp")) {
                        dateHeader.setText(new DateTime(new Date(result.optLong("timestamp"))).toString(DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss (zzz)")));
                    }
                    else {
                    ...
                    for (int j = 0; j < measurementArray.length(); j++)
                    {
                        ...
                        if (singleItem.has("classification_color")) {
                            //System.out.println("COLOR: #"+singleItem.getString("classification_color"));
                            final Drawable d = getResources().getDrawable(android.R.drawable.ic_media_pause).mutate();
                            d.setColorFilter(Color.parseColor("#"+singleItem.getString("classification_color")), PorterDuff.Mode.OVERLAY);
                            itemClassification.setImageDrawable(d); //
                        }
                        else if (singleItem.has("classification")) {
